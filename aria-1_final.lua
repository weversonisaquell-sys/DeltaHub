...
local tabCoils = makeTabButton("Items", 2)
...
local speedToolCard, speedToolState = makeCard(pageCoils, "⚡", "Speed Item", "Clique de novo para remover", 1)
local jumpToolCard, jumpToolState = makeCard(pageCoils, "🦘", "Jump Item", "Clique de novo para remover", 2)
local fusionToolCard, fusionToolState = makeCard(pageCoils, "🟣", "Fusion Item", "Speed + Jump", 3)
...
speedToolCard.MouseButton1Click:Connect(function()
    if speedToolEnabled then
        local removed = removeIfExists("Speed Item")
        speedToolEnabled = false
        speedToolState(false)
        status(removed and "Speed Item removido." or "Speed Item desativado.")
    else
        local ok = makeBuffTool("Speed Item", "speed")
        if ok then
            speedToolEnabled = true
            speedToolState(true)
            status("Speed Item adicionado.")
        end
    end
end)
...
jumpToolCard.MouseButton1Click:Connect(function()
    if jumpToolEnabled then
        local removed = removeIfExists("Jump Item")
        jumpToolEnabled = false
        jumpToolState(false)
        status(removed and "Jump Item removido." or "Jump Item desativado.")
    else
        local ok = makeBuffTool("Jump Item", "jump")
        if ok then
            jumpToolEnabled = true
            jumpToolState(true)
            status("Jump Item adicionado.")
        end
    end
end)
...
fusionToolCard.MouseButton1Click:Connect(function()
    if fusionToolEnabled then
        local removed = removeIfExists("Fusion Item")
        fusionToolEnabled = false
        fusionToolState(false)
        status(removed and "Fusion Item removido." or "Fusion Item desativado.")
    else
        local ok = makeBuffTool("Fusion Item", "fusion")
        if ok then
            fusionToolEnabled = true
            fusionToolState(true)
            status("Fusion Item adicionada.")
        end
    end
end)
...
local function makeSimpleTool(toolName, tooltip, callback)
    local backpack = getBackpack()
    if not backpack then
        status("Backpack não encontrada.")
        return false
    ...
    tool.Activated:Connect(callback)

    return true
end

local f3Card, f3State = makeCard(pageCoils, "F3", "F3 Tool", "Ferramenta de construção", 4)
local resetCard, resetState = makeCard(pageCoils, "↻", "Reset Tool", "Reseta seu personagem", 5)
local bringCard, bringState = makeCard(pageCoils, "◆", "Bring All Tool", "Teleporta todos para você", 6)

local f3Enabled = false
local resetEnabled = false
local bringEnabled = false

f3Card.MouseButton1Click:Connect(function()
    if f3Enabled then
        local removed = removeIfExists("F3 Tool")
        f3Enabled = false
        f3State(false)
        status(removed and "F3 Tool removida." or "F3 Tool desativada.")
    else
        local ok = makeSimpleTool("F3 Tool", "Ferramenta de construção", function()
            status("F3 Tool equipada.")
        end)
        if ok then
            f3Enabled = true
            f3State(true)
            status("F3 Tool adicionada.")
        end
    end
end)
...
resetCard.MouseButton1Click:Connect(function()
    if resetEnabled then
        local removed = removeIfExists("Reset Tool")
        resetEnabled = false
        resetState(false)
        status(removed and "Reset Tool removida." or "Reset Tool desativada.")
    else
        local ok = makeSimpleTool("Reset Tool", "Reseta seu personagem", function()
            local hum = getHumanoid()
            if hum then
                hum.Health = 0
        ...
        if ok then
            resetEnabled = true
            resetState(true)
            status("Reset Tool adicionada.")
        end
    end
end)

bringCard.MouseButton1Click:Connect(function()
    if bringEnabled then
        local removed = removeIfExists("Bring All Tool")
        bringEnabled = false
        bringState(false)
        status(removed and "Bring All Tool removida." or "Bring All Tool desativada.")
    else
        local ok = makeSimpleTool("Bring All Tool", "Teleporta todos para você", function()
            local root = getRoot()
            if not root then
                status("Personagem não encontrado.")
                return
            ...
            status("Jogadores teleportados: " .. tostring(count))
        end)
        if ok then
            bringEnabled = true
            bringState(true)
            status("Bring All Tool adicionada.")
        end
    end
end)
