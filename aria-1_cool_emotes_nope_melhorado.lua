...
local function playNopeEmote()
    ...
    local shoulder = char:FindFirstChild("RightShoulder", true)
    local elbow = char:FindFirstChild("RightElbow", true)
    local wrist = char:FindFirstChild("RightWrist", true)
    local neck = char:FindFirstChild("Neck", true)

    if not shoulder then
        status("Braço direito não encontrado.")
        return
    end

    local joints = {}
    for _, joint in ipairs({shoulder, elbow, wrist, neck}) do
    ...
    task.spawn(function()
        local ok, err = pcall(function()
            -- Levanta a mão e faz três movimentos claros de "não".
            for i = 1, 3 do
                if not shoulder.Parent then return end
                local side = (i % 2 == 1) and -1 or 1
                local base = joints[shoulder]
                shoulder.C0 = base * CFrame.Angles(math.rad(-65), math.rad(15), math.rad(side * 45))
                if wrist and joints[wrist] then
                    wrist.C0 = joints[wrist] * CFrame.Angles(math.rad(-20), math.rad(side * 20), math.rad(side * 15))
                end
                task.wait(0.16)
            ...
            -- Finaliza apontando a mão para baixo, simulando o dislike.
            local base = joints[shoulder]
            shoulder.C0 = base * CFrame.Angles(math.rad(35), math.rad(10), math.rad(5))
            if elbow and joints[elbow] then
                elbow.C0 = joints[elbow] * CFrame.Angles(math.rad(-45), 0, 0)
            end
            if wrist and joints[wrist] then
                wrist.C0 = joints[wrist] * CFrame.Angles(math.rad(70), 0, 0)
            end
            task.wait(0.55)
        end)
