...
main.Name = "Window"
main.Size = UDim2.fromOffset(640, 640)
main.Position = UDim2.new(0.5, -320, 0.5, -320)
main.BackgroundColor3 = Color3.fromRGB(20, 17, 29)
main.BorderSizePixel = 0
main.Active = true
main.Draggable = false
main.Parent = gui

-- Responsive sizing: keeps the entire window inside phones, tablets and landscape games.
local function fitWindow()
    ...
    local width = math.max(320, math.min(640, viewport.X - margin * 2))
    local height = math.max(420, math.min(760, viewport.Y - margin * 2))

    -- If the device is shorter than the minimum, prefer fitting the screen.
    width = math.min(width, math.max(1, viewport.X - 16))
    height = math.min(height, math.max(1, viewport.Y - 16))
...
htmlStatus.Name = "HtmlStatusBar"
htmlStatus.Parent = main
htmlStatus.AnchorPoint = Vector2.new(0, 1)
htmlStatus.Position = UDim2.new(0, 0, 1, 0)
htmlStatus.Size = UDim2.new(1, 0, 0, 64)
htmlStatus.BackgroundColor3 = Color3.fromRGB(29, 25, 39)
...
statusLine.BackgroundColor3 = Color3.fromRGB(43, 36, 55)
...
htmlStatusText.Position = UDim2.fromOffset(24, 0)
htmlStatusText.Size = UDim2.new(1, -48, 1, 0)
htmlStatusText.Font = Enum.Font.Gotham
htmlStatusText.TextSize = 16
htmlStatusText.TextColor3 = Color3.fromRGB(178, 169, 197)
htmlStatusText.TextXAlignment = Enum.TextXAlignment.Left
htmlStatusText.Text = "⟳  Updated — Instant updates"
...
stroke.Color = Color3.fromRGB(49, 40, 63)
stroke.Thickness = 1
stroke.Parent = main

local titleBar = Instance.new("Frame")
titleBar.Size = UDim2.new(1, 0, 0, 76)
titleBar.BackgroundColor3 = Color3.fromRGB(29, 25, 39)
...
title.Text = "Executor Loader"
title.TextSize = 20
title.TextColor3 = Color3.fromRGB(188, 181, 204)
title.TextXAlignment = Enum.TextXAlignment.Center
title.Parent = titleBar

local function winBtn(txt, xOff, hoverColor, hoverText)
    local b = Instance.new("TextButton")
    b.Size = UDim2.fromOffset(48, 76)
    ...
    b.MouseLeave:Connect(function()
        b.BackgroundTransparency = 1
        b.TextColor3 = Color3.fromRGB(188, 181, 204)
    end)
...
backBtn.Size = UDim2.fromOffset(52, 76)
backBtn.Position = UDim2.fromOffset(14, 0)
backBtn.BackgroundTransparency = 1
backBtn.BorderSizePixel = 0
backBtn.Text = "←"
backBtn.Font = Enum.Font.Gotham
backBtn.TextSize = 30
backBtn.TextColor3 = Color3.fromRGB(170, 162, 190)
backBtn.AutoButtonColor = false
backBtn.Parent = titleBar

local refreshBtn = Instance.new("TextButton")
refreshBtn.Name = "RefreshButton"
refreshBtn.Size = UDim2.fromOffset(52, 76)
refreshBtn.Position = UDim2.fromOffset(66, 0)
refreshBtn.BackgroundTransparency = 1
refreshBtn.BorderSizePixel = 0
refreshBtn.Text = "⟳"
refreshBtn.Font = Enum.Font.Gotham
refreshBtn.TextSize = 28
refreshBtn.TextColor3 = Color3.fromRGB(170, 162, 190)
refreshBtn.AutoButtonColor = false
refreshBtn.Parent = titleBar

local function subtleHover(button)
    button.MouseEnter:Connect(function()
        button.TextColor3 = Color3.fromRGB(226, 219, 240)
    end)
    button.MouseLeave:Connect(function()
        button.TextColor3 = Color3.fromRGB(170, 162, 190)
    end)
end

subtleHover(backBtn)
subtleHover(refreshBtn)

local closeBtn = winBtn("✕", -48, Color3.fromRGB(232, 17, 35), Color3.fromRGB(255,255,255))
local minBtn = winBtn("—", -96, Color3.fromRGB(232,232,232))


-- Mouse + touch drag from the title bar. It is clamped to the visible screen.
do
    ...
    local function clampPosition(x, y)
        ...
        local minX = -size.X + padding
        local maxX = viewport.X - padding
        local minY = -size.Y + padding
        local maxY = viewport.Y - padding
    ...
    UIS.InputChanged:Connect(function(input)
        if dragging and (input.UserInputType == Enum.UserInputType.MouseMovement
            ...
            local x, y = clampPosition(startPos.X + delta.X, startPos.Y + delta.Y)
...
sidebar.Size = UDim2.new(0, 104, 1, 0)
sidebar.BackgroundColor3 = Color3.fromRGB(22, 19, 31)
sidebar.BorderSizePixel = 0
sidebar.Parent = body

local sidebarStroke = Instance.new("UIStroke")
sidebarStroke.Color = Color3.fromRGB(42, 34, 51)
sidebarStroke.Parent = sidebar

local pages = Instance.new("Frame")
pages.Position = UDim2.new(0, 104, 0, 0)
pages.Size = UDim2.new(1, -104, 1, 0)
pages.BackgroundTransparency = 1
pages.ClipsDescendants = true
pages.Parent = body

local sidebarLayout = Instance.new("UIListLayout")
sidebarLayout.Padding = UDim.new(0, 22)
sidebarLayout.SortOrder = Enum.SortOrder.LayoutOrder
sidebarLayout.Parent = sidebar

local sidebarPad = Instance.new("UIPadding")
sidebarPad.PaddingTop = UDim.new(0, 30)
sidebarPad.PaddingLeft = UDim.new(0, 18)
sidebarPad.PaddingRight = UDim.new(0, 18)
...
local function makePage(name)
    ...
    frame.BackgroundColor3 = Color3.fromRGB(20, 17, 29)
    frame.BorderSizePixel = 0
    frame.Visible = false
    frame.Parent = pages
    frame.ScrollBarThickness = 5
    frame.ScrollBarImageColor3 = Color3.fromRGB(106, 54, 178)
...
pageMain.CanvasPosition = Vector2.new(0, 0)

local function makeTabButton(text, order)
    local b = Instance.new("TextButton")
    b.Size = UDim2.new(1, -36, 0, 64)
    b.BackgroundColor3 = Color3.fromRGB(29, 26, 40)
    b.BorderSizePixel = 0
    b.Text = text
    b.TextSize = 28
    b.Font = Enum.Font.GothamMedium
    b.TextColor3 = Color3.fromRGB(181, 173, 198)
    b.AutoButtonColor = false
    b.LayoutOrder = order
    b.Parent = sidebar
...
local tabMain = makeTabButton("⌂", 1)
local tabCoils = makeTabButton("◈", 2)
local tabCool = makeTabButton("☄", 3)
local tabEmotes = makeTabButton("☺", 4)
...
local function styleTabs(activeButton)
    for _, b in ipairs({tabMain, tabCoils, tabCool, tabEmotes}) do
        local on = b == activeButton
        b.BackgroundColor3 = on and Color3.fromRGB(60, 29, 90) or Color3.fromRGB(29, 26, 40)
        b.TextColor3 = on and Color3.fromRGB(218, 195, 255) or Color3.fromRGB(181, 173, 198)
        local st = b:FindFirstChildOfClass("UIStroke")
        if on and not st then
            st = Instance.new("UIStroke")
            st.Thickness = 1
            st.Parent = b
        end
        if st then
            st.Color = Color3.fromRGB(124, 62, 186)
...
refreshBtn.MouseButton1Click:Connect(function()
    for _, page in ipairs({pageMain, pageCoils, pageCool, pageEmotes}) do
...
local function makeCard(parent, icon, name, desc, order)
    ...
    card.BackgroundColor3 = Color3.fromRGB(27, 23, 36)
    ...
    s.Color = Color3.fromRGB(35, 30, 45)
    s.Thickness = 1
    s.Parent = card

    local ic = Instance.new("TextLabel")
    ic.BackgroundTransparency = 1
    ic.Position = UDim2.fromOffset(22, 0)
    ic.Size = UDim2.fromOffset(48, 92)
    ic.Text = icon
    ic.TextSize = 28
    ic.Font = Enum.Font.GothamBold
    ic.TextColor3 = Color3.fromRGB(193, 60, 255)
    ic.Parent = card

    local nm = Instance.new("TextLabel")
    nm.BackgroundTransparency = 1
    nm.Position = UDim2.fromOffset(78, 18)
    nm.Size = UDim2.new(1, -170, 0, 24)
    nm.Font = Enum.Font.GothamMedium
    nm.Text = name
    nm.TextSize = 18
    nm.TextColor3 = Color3.fromRGB(242, 238, 251)
    ...
    ds.Position = UDim2.fromOffset(78, 46)
    ds.Size = UDim2.new(1, -170, 0, 22)
    ds.Font = Enum.Font.Gotham
    ds.Text = desc
    ds.TextSize = 14
    ds.TextColor3 = Color3.fromRGB(168, 159, 189)
    ds.TextXAlignment = Enum.TextXAlignment.Left
    ds.TextTruncate = Enum.TextTruncate.AtEnd
    ds.Parent = card

    local pill = Instance.new("Frame")
    pill.Size = UDim2.fromOffset(58, 32)
    pill.Position = UDim2.new(1, -78, 0.5, -16)
    pill.BackgroundColor3 = Color3.fromRGB(48, 45, 64)
    ...
    knob.BackgroundColor3 = Color3.fromRGB(180, 176, 200)
    ...
    card.MouseEnter:Connect(function()
        TweenService:Create(card, TweenInfo.new(0.12), {
            BackgroundColor3 = Color3.fromRGB(30, 28, 43)
        }):Play()
    end)

    card.MouseLeave:Connect(function()
        TweenService:Create(card, TweenInfo.new(0.12), {
            BackgroundColor3 = Color3.fromRGB(27, 23, 36)
        }):Play()
    end)

    local function setState(on)
        TweenService:Create(pill, TweenInfo.new(0.15), {
            BackgroundColor3 = on and Color3.fromRGB(124, 76, 220) or Color3.fromRGB(48, 45, 64)
        }):Play()

        TweenService:Create(knob, TweenInfo.new(0.15), {
            Position = on and UDim2.fromOffset(31, 5) or UDim2.fromOffset(5, 5),
            BackgroundColor3 = on and Color3.fromRGB(255,255,255) or Color3.fromRGB(180, 176, 200)
        }):Play()
    end
...
statusBar.TextColor3 = Color3.fromRGB(150,145,170)
